# DMS One Flex MCP – Telepítés Windowson

Lépésről lépésre útmutató a DMS One Flex bővítmény telepítéséhez **Windows** alatt,
Claude Desktopban. Két út van: **A) egykattintásos (.mcpb) – ajánlott**, és
**B) manuális (Node.js) – fejlesztőknek**.

> A `.mcpb` csomag platformfüggetlen: ugyanez a fájl megy macOS-en és Linuxon is.
...
`https://claude.ai/download` és telepítsd.

### 3. lépés – Extensions megnyitása
Bal alul kattints a **neved / profil ikon** → **Settings** (Beállítások) →
...
Csomagold ki a projektet (pl. `C:\Tools\dmsone-flex\`), majd PowerShellben:
```powershell
cd "C:\Tools\dmsone-flex\Flex"
npm install
npm run build
```
...
```
> Tipp: a Futtatás (Win+R) ablakba írd: `%APPDATA%\Claude` → Enter.

Illeszd be (a `dist\index.js` **teljes** elérési útjával, **dupla** visszaperjellel):
```json
{
  "mcpServers": {
    "dmsone-flex": {
      "command": "node",
      "args": ["C:\\Tools\\dmsone-flex\\Flex\\dist\\index.js"],
      "env": {
...
Zárd be teljesen (a tálca jobb alsó sarkában is **Quit**), majd indítsd újra.
A Flex eszközök ezután elérhetők.
...
| (Manuális) nem töltődnek be az eszközök | Ellenőrizd a `claude_desktop_config.json` elérési útját és a **dupla visszaperjeleket**. |
